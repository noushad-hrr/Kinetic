...
@Component({
  ...
  template: `
    <div class="p-4 max-w-7xl mx-auto space-y-4">
              ...
              <tr class="border-b border-slate-50 hover:bg-slate-50/60 transition-colors">
                ...
                <td class="px-3 py-2.5 text-right">
                  <div class="flex items-center justify-end gap-0.5 flex-wrap">
                    <a [routerLink]="['/tasks-manager/tasks']" [queryParams]="{ project: p.id, view: 'day' }"
      ...
      <p class="text-2xs text-slate-400">Showing {{ filtered().length }} of {{ ws.projects().length }} projects</p>
    </div>
      ...
      @if (projectForm) {
        <form [formGroup]="projectForm" class="space-y-3">
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-2xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Start</label>
              <input formControlName="start" type="text" placeholder="Mar 01"
                     class="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/25 focus:border-primary">
            </div>
            <div>
              <label class="block text-2xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Due</label>
...
export class TmProjectsComponent {
  ...
  statusClass(s: string): string {
    return { Active: 'bg-emerald-100 text-emerald-800', Triage: 'bg-slate-100 text-slate-700',
             'On Hold': 'bg-amber-100 text-amber-800', Completed: 'bg-stone-200 text-stone-800' }[s] ?? 'bg-slate-100 text-slate-600';
  }
  ...
  saveProject() {
    ...
    if (cur) {
      ...
      this.router.navigate(['/tasks-manager/tasks'], { queryParams: { project: p.id, view: 'day' } });
    }
